...
describe('RewardsManager', function () {
    ...
    describe('admin proxy and wrapper functions', function () {
        describe('pause views (isTokenMintPaused, isClaimRewardPaused)', function () {
            it('isTokenMintPaused and isClaimRewardPaused reflect server state', async function () {
                const { manager, managerWallet, user1, mockERC20 } = await loadFixture(deployWithServerAndTokenFixture);
                const tokenId = 1;
                const rewardToken = {
                    tokenId,
                ...
                await manager.connect(managerWallet).createTokenAndDepositRewards(SERVER_ID, rewardToken, { value: 0 });
                expect(await manager.isTokenMintPaused(SERVER_ID, tokenId)).to.be.false;
                expect(await manager.isClaimRewardPaused(SERVER_ID, tokenId)).to.be.false;
                await manager.connect(managerWallet).updateTokenMintPaused(SERVER_ID, tokenId, true);
                expect(await manager.isTokenMintPaused(SERVER_ID, tokenId)).to.be.true;
                await manager.connect(managerWallet).updateClaimRewardPaused(SERVER_ID, tokenId, true);
                expect(await manager.isClaimRewardPaused(SERVER_ID, tokenId)).to.be.true;
            });
        });
        ...
        describe('getServerSigners', function () {
            ...
            it('returns signers after addWhitelistSigner', async function () {
                ...
                await manager.connect(user1).addWhitelistSigner(SERVER_ID, user1.address);
                list = await manager.getServerSigners(SERVER_ID);
                expect(list.length).to.equal(2);
                expect(list).to.include(user2.address);
                expect(list).to.include(user1.address);
                await manager.connect(user1).removeWhitelistSigner(SERVER_ID, user2.address);
                list = await manager.getServerSigners(SERVER_ID);
                expect(list.length).to.equal(1);
        ...
        describe('reduceRewardSupply', function () {
            it('MANAGER_ROLE can reduce reward supply and event is emitted', async function () {
                const { manager, managerWallet, mockERC20 } = await loadFixture(deployWithServerAndTokenFixture);
                const tokenId = 1;
                const rewardToken = {
                    tokenId,
                ...
                await expect(manager.connect(managerWallet).reduceRewardSupply(SERVER_ID, tokenId, 3))
                    .to.emit(manager, 'RewardSupplyChanged')
                    .withArgs(SERVER_ID, tokenId, 10, 7);
