...
describe('SoulBoundBadges', function () {
  ...
  describe('Token Exists', () => {
    it('should fail to mint if putting wrong token id and vice versa', async function () {
      ...
      await expect(soulBoundBadges.mint(playerAccount.address, 1, 1, true)).to.be.rejectedWith('TokenNotExist()');
  ...
  describe('Pause Mint', () => {
    it('should fail to mint if contract is paused and vice versa', async function () {
      ...
      await expect(soulBoundBadges.mint(playerAccount.address, 22, 1, true)).to.be.revertedWith('Pausable: paused');
  ...
  describe('Mint', () => {
    ...
    it('fail if try to mint more than the limit', async function () {
      ...
      await expect(soulBoundBadges.mint(playerAccount.address, tokenId, 2, true)).to.be.rejectedWith('ExceedMaxMint()');
    });
    ...
    it('fail if already minted', async function () {
      ...
      await expect(soulBoundBadges.mint(playerAccount.address, tokenId, 1, true)).to.be.rejectedWith('AlreadyMinted()');
    });

    it('fail if try to mint invalid tokenId', async function () {
      const tokenId = 30;

      await expect(soulBoundBadges.mint(playerAccount.address, tokenId, 1, true)).to.be.rejectedWith('TokenNotExist()');
    });

    it('fail sender has no minter role', async function () {
      await expect(soulBoundBadges.connect(playerAccount).mint(playerAccount.address, tokenId, 1, true)).to.be.revertedWith(
  ...
  describe('BatchMint', () => {
    ...
    it('fail if try to mint more than the limit', async function () {
      ...
      await expect(soulBoundBadges.mintBatch(playerAccount.address, [1, 2, 3], [100, 200, 300], false)).to.be.rejectedWith('ExceedMaxMint()');
    });
    ...
    it('fail if already minted', async function () {
      ...
      await expect(soulBoundBadges.mintBatch(playerAccount.address, [1, 2, 3], [1, 1, 1], true)).to.be.rejectedWith('AlreadyMinted()');
    });

    it('fail if try to mint invalid tokenId', async function () {
      await expect(soulBoundBadges.mintBatch(playerAccount.address, [33, 299], [1, 1], true)).to.be.rejectedWith('TokenNotExist()');
    });

    it('fail sender has no minter role', async function () {
      await expect(soulBoundBadges.connect(playerAccount).mintBatch(playerAccount.address, [1, 2, 3], [100, 200, 300], false)).to.be.revertedWith(
        `AccessControl: account ${playerAccount.address.toLowerCase()} is missing role 0x9f2df0fed2c77648de5860a4cc508cd0818c85b8b8a1ab4ceeef8d981c8956a6`
      );
    });
  });
  ...
  describe('Token URI', () => {
    it('Get Uri() should fail if tokenId not exists', async function () {
      const tokenId = 1;
      await expect(soulBoundBadges.uri(tokenId)).to.be.rejectedWith('TokenNotExist()');
    });
    ...
    it('updateBaseUri() should fail to update new baseuri if has no manager role', async function () {
      ...
      await expect(soulBoundBadges.connect(playerAccount).updateBaseUri(newBaseURI)).to.be.revertedWith(
