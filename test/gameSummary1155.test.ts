...
describe.only('GameSummary1155', function () {
    ...
    let defaultCommonGameSummary: GameSummary1155.GameSummaryStruct;
    ...
    it.only('As Player must mint game summary achievement', async function () {
        ...
        const tx = await gameSummary1155
            ...
            .mintGameSummaryWithSignature(DEFAULT_GAME_ID, randomAchievementIds.length, DEFAULT_STORE_ID, nonce, signature);
        await tx.wait();
        const tokenId = await gameSummary1155.getTokenId(DEFAULT_STORE_ID, DEFAULT_GAME_ID);
    ...
    it('As Admin must mint a game summary for a player', async function () {
        ...
        await tx.wait();
        const balance = await gameSummary1155.balanceOf(playerAccount.address, hashIds(DEFAULT_STORE_ID, DEFAULT_GAME_ID));
    ...
    it('As Admin must mint a game summary but not the same game summary twice', async function () {
        ...
        await expect(
            gameSummary1155.adminMintGameSummary(playerAccount.address, DEFAULT_GAME_ID, randomAchievementIds.length, DEFAULT_STORE_ID, true)
            // @ts-ignore-next-line
    ...
    it('The pause functionality should works as expected', async function () {
        ...
        await (
            await await gameSummary1155.createCommonGameSummary(
                DEFAULT_STORE_ID,
                DEFAULT_GAME_ID + 2,
                'GameTest1234',
                'test://on_chain_uri',
    ...
    it('As admin must mint a batch of game summaries achievement for players', async function () {
        ...
        for await (const gameID of GAME_IDS) {
            await (
                await gameSummary1155.createCommonGameSummary(DEFAULT_STORE_ID, gameID, 'GameTest1234', 'test://on_chain_uri', 'external_uri', 9999999999)
            ).wait();
        }
    ...
    it('As an User I can get 1 GameSummary that the admin minted per game and check the PlayerData', async function () {
        ...
        await (
            await gameSummary1155.createCommonGameSummary(
                DEFAULT_STORE_ID + 1,
                DEFAULT_GAME_ID,
                'GameTest1234',
                'test://on_chain_uri',
        ...
        const tx = await gameSummary1155.adminMintGameSummary(playerAccount.address, DEFAULT_GAME_ID, PLAYER_ACHIEVEMENTS_LENGTH, DEFAULT_STORE_ID + 1, true);
        await tx.wait();
        const gameSummary = await gameSummary1155.connect(playerAccount).getGameSummary(hashIds(DEFAULT_STORE_ID + 1, DEFAULT_GAME_ID));
        expect(Number(gameSummary.gameId)).to.equal(DEFAULT_GAME_ID);
        expect(Number(gameSummary.totalAchievements)).to.equal(TOTAL_ACHIEVEMENTS_IN_GAME);
        const playerData = await gameSummary1155
            .connect(playerAccount)
            .getPlayerGameData(playerAccount.address, hashIds(DEFAULT_STORE_ID + 1, DEFAULT_GAME_ID));
    ...
    it('If the admin update one GameSummary, everyone will have this update', async function () {
        ...
        const tx = await gameSummary1155.adminMintGameSummary(playerAccount.address, DEFAULT_GAME_ID, 20, DEFAULT_STORE_ID + 1, true);
        await tx.wait();
        const gameSummary = await gameSummary1155.connect(playerAccount).getGameSummary(hashIds(DEFAULT_STORE_ID + 1, DEFAULT_GAME_ID));
        expect(gameSummary.image).to.equal('test://on_chain_uri');
        await gameSummary1155.updateCommonGameSummary(hashIds(DEFAULT_STORE_ID + 1, DEFAULT_GAME_ID), 'New_NAME', 'test://on_chain_uri_2', 'external_uri', 577);
        const gameSummaryUpdated = await gameSummary1155.connect(playerAccount).getGameSummary(hashIds(DEFAULT_STORE_ID + 1, DEFAULT_GAME_ID));
    ...
    it('As user I cant transfer any SBT using the safeBatchTransferFrom', async function () {
        ...
        const sbt1Trx = await gameSummary1155.adminMintGameSummary(playerAccount.address, GAME_IDS[0], 1, DEFAULT_STORE_ID, false);
        const sbt2Trx = await gameSummary1155.adminMintGameSummary(playerAccount.address, GAME_IDS[1], 1, DEFAULT_STORE_ID, true);
        const sbt3Trx = await gameSummary1155.adminMintGameSummary(playerAccount.address, GAME_IDS[2], 1, DEFAULT_STORE_ID, false);
        const sbt4Trx = await gameSummary1155.adminMintGameSummary(playerAccount.address, GAME_IDS[3], 1, DEFAULT_STORE_ID, false);
        await sbt1Trx.wait();
        await sbt2Trx.wait();
        await sbt3Trx.wait();
        await sbt4Trx.wait();
    ...
    it('As a user I could burn any not SBT', async function () {
        ...
        const burnTx = await gameSummary1155.connect(playerAccount).burn(TOKEN_ID_1);
        await burnTx.wait();
    ...
    it('As a user I can burn any SBT but the info of the player will be cleared', async function () {
        ...
        await sbt1Trx.wait();
        await (await gameSummary1155.connect(playerAccount).burn(TOKEN_ID_1)).wait();
    ...
    it('should allow batch burning of tokens', async function () {
        ...
        await await gameSummary1155.createCommonGameSummary(DEFAULT_STORE_ID, DEFAULT_GAME_ID + 1, 'testGame', 'ipfs://hash', 'https://cdn.comg/jpg', 200);
        ...
        const burnTx = await gameSummary1155.connect(playerAccount).burnBatch([TOKEN_ID_1, TOKEN_ID_2]);
        await burnTx.wait();
    ...
    it('as admin should update the qty of achievements for a player', async function () {
        ...
        const playerDataUpdated = await gameSummary1155
            .connect(playerAccount)
            .getPlayerGameData(playerAccount.address, hashIds(DEFAULT_STORE_ID, DEFAULT_GAME_ID));
    ...
    it('should update the qty of achievements for a player using the signature', async function () {
        ...
        const { signature: signature2, nonce: nonce2 } = await generateSignature({ walletAddress: playerAccount.address, signer: minterAccount });
    ...
    it('the batchUpdate using the signature should works as expected', async function () {
        ...
        const updateTx = await gameSummary1155.connect(playerAccount).batchPlayerUpdateAchievementsWithSignature(TOKEN_IDS, [2, 90, 20], nonce2, signature2);
        await updateTx.wait();
    ...
    it("As admin or user minting shouldn't have collisions between ids", async function () {
        ...
        await (await await gameSummary1155.createCommonGameSummary(STORE_ID, GAME_ID, 'GameTest1234', 'test://on_chain_uri', 'external_uri', 500)).wait();
