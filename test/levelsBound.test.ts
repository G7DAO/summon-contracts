...
describe('LevelsBound', function () {
    ...
    it("As user I can't mint levels for a player", async function () {
        await expect(levelsBound.connect(playerAccount).levelUp(playerAccount.address, 1)).to.be.rejectedWith(
            `VM Exception while processing transaction: reverted with custom error 'OwnableUnauthorizedAccount("${playerAccount.address}")`
        );
    });
