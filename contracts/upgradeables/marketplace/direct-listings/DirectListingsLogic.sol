...
import "../offers/OffersStorage.sol";
...
//    }
    ...
    function buyFromListingWithOffer(
        ...
        require(_msgSender()  == offer.offeror || _msgSender() == listing.listingCreator, "Marketplace: not offeror or listing creator.");
        ...
        if (_directListingsStorage().currencyPriceForListing[_listingId][offer.currency] > 0) {
            targetTotalPrice = offer.quantity * _directListingsStorage().currencyPriceForListing[_listingId][
                offer.currency
            ];
        } else {
