...
contract AchievementFacet is ERC1155, AccessControl, Pausable {
    ...
    function safeTransferFrom(
        ...
        if (astore.tokenData[_id].soulbounded) {
            require(
                astore.tokenData[_id].owner == _from,
                "AchievementFacet: Cannot transfer soulbound token"
            );
        }
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < _ids.length; ++i) {
            if (astore.tokenData[_ids[i]].soulbounded) {
                require(
                    astore.tokenData[_ids[i]].owner == _from,
                    "AchievementFacet: Cannot transfer soulbounded token"
                );
            }
        }
